---
title: "Articulo cafe"
author: "Sebastian Fuentes González B83063"
date: "7/8/2021"
...
```{r}
options(contrasts = c("contr.sum","contr.poly"))
mod_prueba = lm(calificacion ~ cafe + filtro , data = base, weights = w)
anova(mod_prueba)

model_tables = model.tables(aov(mod_prueba))
model_tables
betas = model_tables$tables$filtro

SCFil = 10*sum(betas^2)
CMFil = SCFil/(3-1)
CMRes = anova(mod_prueba)[3,3]

f = CMFil/CMRes
n = nrow(base)
k = length(mod_prueba$coefficients)

pf(f,(3-1),(n-k),lower.tail = F)  # = 2.748153e-30 con café primero y 3.13909e-29 de segundo , entonces se llega a la misma conclusión que sin MCP
pf(f,(3-1),(n-k),lower.tail = F)<0.05
```

```{r}

# SI cambian, entonces preguntar al profe
m = tapply(base$calificacion,base$filtro,mean)
d_tm = m[3]-m[1]
d_tp = m[3]-m[2]
d_mp = m[1]-m[2]

CMRes.1 = anova(mod_prueba)[3,3] 
ee = sqrt(2*(CMRes.1)/10)
